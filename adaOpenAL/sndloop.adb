...
accept init(sid: interfaces.c.int; sfil: string ) do
...
-- "protected" and runs "single-threaded"
	...
	oal_hpp.bufferdata(
		...
		dsize,
		samprate);
	oal_hpp.gensources(ssid);
	oal_hpp.sourcei( ssid );
...
------------------------------------------------------midLoop-----------
				...
				oal_hpp.sourceplay(ssid);
				inner:loop

					select
						accept Stop;
						oal_hpp.sourcestop(ssid);
						exit mid;
					or
						delay 0.1;
						if oal_hpp.is_stopped(ssid)=oal_hpp.is_true then
							oal_hpp.sourceplay(ssid);
						end if;
					end select;
