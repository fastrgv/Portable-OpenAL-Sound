...
use Interfaces.C.Strings;
   ...
   procedure termSnds is
	...
	-- lvl>=0.0 ...
	-- lvl=0.0 => silent; 
	-- lvl=1.0 => UnAttenuated; 
	-- lvl>1.0 => even louder
	procedure setvolume(nbuf: int; lvl: float) is
		lev: float := lvl;
	begin
		if lev<0.0 then lev:=0.0; end if;
		oal_hpp.setVolume(nbuf, lev);
	end setvolume;
